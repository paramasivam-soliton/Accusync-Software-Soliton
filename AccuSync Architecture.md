...
**A View-Model injects a service interface (`IUserService`), never a repository interface (`IUserRepository`), directly.** Repositories are plain persistence — no encryption, hashing, or validation — and are meant to be called only by the matching `AccuSync.Application` service, which owns that business logic. A View-Model that reaches past the service and calls the repository directly silently loses whatever the service enforces (e.g. a user's encrypted fields no longer get encrypted/decrypted transparently). `AccuSync.EF/UserRepository.cs` and `AccuSync.Application/Services/Users/UserService.cs` are the reference example: `LoginViewModel`/`ChangePasswordViewModel` depend on `IUserService` only.
