...
Login epic — [ASWD-1](https://natus-jira.atlassian.net/browse/ASWD-1).

# 2. Links to reference material

- AccuSync Software Requirements Specification, DOC-076814 — referenced sections: failed-attempt
  lockout threshold, configurable lockout duration, administrator override/unlock.
...
`System.Security.Cryptography`, built into .NET — no third-party dependency), at 600,000
...
| `ProfileId` | `int` | Foreign key into `Profiles`, interpreted as the account's role — see Role-based access control below. |
...
default (least-privileged) value. A `Profiles` table (`ProfileId` int primary key, `Name`) holds
one row per role, with `ProfileId` values defined to equal the matching `UserRole` member exactly
(`0` = Screener, `1` = Admin) and seeded at migration time. `User.ProfileId` is a foreign key into
this table; resolving a stored `ProfileId` to a `UserRole` is a direct, checked cast (any value
with no defined `UserRole` member — including one with no matching `Profiles` row — resolves to
`Screener`) rather than a string comparison.

This supersedes the original decision to reuse `ProfileId` as a plain, unvalidated string column
(see Alternative 5 below) — that approach left the role signal both unvalidated at the database
level and duplicated as a raw string literal wherever it was compared. `Profiles` here still holds
only an id and a name; the richer, permission-bundle model described in Open issues remains future
work and was deliberately not pulled into this table.
...
- `AccuSync.Core` — `User`, `Profile`, and `AppSettings` entities; `UserRole` enum; authentication,
...
- `AccuSync.EF` — user and app-settings repositories and EF configurations; schema migrations
  for the username blind index, the `Status`→`IsActive` column (type and rename), the new
  `AppSettings` table, and the new `Profiles` table (with `User.ProfileId` changed from a string
  to an FK into it).
...
4. **Deterministic or order-preserving encryption of the username field**, to keep it directly
   ...
   semantic security.
5. **Reusing `ProfileId` as a plain, unvalidated string column** (`"Admin"` / `"Screener"`), with
   no supporting table — the design's original approach, since superseded by the `Profiles` table
   described in Role-based access control above. Revisited after review: an unvalidated string
   column allowed a role value to be persisted with nothing to enforce that it was ever recognized,
   and the recognized value ("Admin") existed only as a string literal duplicated at each
   comparison site rather than tied to the `UserRole` enum. A minimal `Profiles` table (id + name)
   addresses both without pulling in the full, richer permission-bundle model described in Open
   issues, which remains future work.
...
- The existing 90-day password-expiration check itself is retained unchanged by this design;
  enforcing it end-to-end at login is covered under Later work under this epic, below.
...
# 6. Later work under this epic

Further work under the Login epic built on the design established here rather than introducing a
parallel approach:

- The 90-day password-expiration check is now enforced end-to-end: an account whose password has
  aged out is redirected into the same reset flow described under Password complexity and reuse
  above, which continues to enforce the existing complexity and reuse-history rules.
- A change-password screen, reachable from application settings once a user is signed in, enforces
  the same complexity and reuse-history rules described above at the point of a self-initiated
  password change, rather than only at forced/first-login changes.
- The login and change-password windows were made draggable and independently closable. This is a
  window-management usability change and does not alter any authentication or session logic
